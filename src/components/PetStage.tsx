    ...
    switch (level) {
      case 'Healthy':
        return {
          ...
          auraShadow: '0 0 45px rgba(16, 185, 129, 0.3)',
        };
      case 'Attention':
        return {
          ...
          auraShadow: '0 0 45px rgba(245, 158, 11, 0.35)',
        };
      case 'Blocked':
        return {
    ...
    >
      ...
      <div className="relative z-10 flex flex-wrap items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs text-slate-500 hidden sm:inline-block font-medium">
            ...
            {state.primarySymptom === 'detached_head' && '🧭 Lost wandering compass'}
            {state.primarySymptom === 'stale_branch' && '💤 Merged & snoozing'}
            {state.primarySymptom === 'clean_sync' && '✨ Sparkling in-sync'}
          </span>
        </div>

        {/* Health Progress Bar */}
        <div className="flex items-center gap-2.5 min-w-[170px]">
          <span className="text-xs font-medium text-slate-500">Repo Health</span>
          <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200/60 p-0.5">
            <motion.div
      ...
      <div className="relative z-10 flex flex-col items-center justify-center py-4 sm:py-6">
        ...
        <div className="mt-3 text-center max-w-md">
          <h3 className="text-sm font-semibold text-slate-900 flex items-center justify-center gap-1.5">
        ...
        {/* Quick Stage Controls for Image Studio & Voice */}
        <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
          {customAvatarUrl && (
            <button
              ...
              className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-lg border border-slate-200 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
          ...
          {onOpenImageStudio && (
            <button
              ...
              className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-semibold rounded-lg border border-rose-200 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
              <Wand2 className="w-3.5 h-3.5 text-rose-600" />
              <span>Mascot Image Studio</span>
            </button>
          )}
          ...
          {onOpenVoiceModal && (
            <button
              ...
              className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-semibold rounded-lg border border-indigo-200 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
              <Mic className="w-3.5 h-3.5 text-indigo-600" />
              <span>Talk with Byte</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between text-[11px] text-slate-400 border-t border-slate-100 pt-3 mt-1">
...
const PetGraphic: React.FC<PetGraphicProps> = ({ symptom, healthLevel, isHovered }) => {
  ...
  return (
    ...
    >
      <svg
        viewBox="0 0 200 180"
        className="w-48 h-44 drop-shadow-sm overflow-visible"
      >
        <defs>
